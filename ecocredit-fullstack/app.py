...
def create_item():
    ...
    data = request.get_json(silent=True) or {}
    required = ("title", "category", "condition", "location", "points")
    if any(not str(data.get(field, "")).strip() for field in required):
        return jsonify({"error": "Please complete all item details."}), 400
    with closing(get_db()) as db:
        db.execute("""INSERT INTO items (title, category, item_condition, location, points, emoji, description, owner_name)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
                   (data["title"].strip(), data["category"], data["condition"].strip(), data["location"].strip(),
                    int(data["points"]), data.get("emoji", "♻️"), data.get("description", "A useful campus item."), user["name"]))
...
def request_swap(item_id: int):
    ...
    with closing(get_db()) as db:
        ...
        db.commit()
    return jsonify({"message": "Swap request sent! Arrange a safe on-campus meeting after the owner accepts."}), 201
